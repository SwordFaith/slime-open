...
class TestTokenLogprobCaching:
    ...
    @pytest.mark.e2e
    def test_verify_tokens_cached_on_first_request(
    ...
    def test_partial_cache_hit(
        ...
        response1 = client.post("/v1/chat/completions", json=request_data1)
        assert response1.status_code == 200
        ...
        start_time = time.time()
        response2 = client.post("/v1/chat/completions", json=request_data2)
        time2 = time.time() - start_time
